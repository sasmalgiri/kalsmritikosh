    ...
    private func runLLM(
        ...
        if let fmProvider = provider as? FoundationModelsProvider {
            do {
                ...
                AtlasLog.brain.info("expert.timeline LLM: produced \(typed.count) typed claims via @Generable")
                return (typed, 0)
            } catch {
                AtlasLog.brain.error("expert.timeline LLM: typed path failed (\(String(describing: error), privacy: .public)); falling back to prompt-parse")
            }
        }
        do {
