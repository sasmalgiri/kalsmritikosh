...
public struct FinancialExpert: Expert {
    ...
    private func runLLM(
        ...
        guard let provider = try? await capabilities.resolve(spec) else {
            AtlasLog.brain.info("expert.financial LLM: no provider resolved for spec; using heuristic fallback")
            return ([], 0)
        }
        guard await provider.isAvailable() else {
            AtlasLog.brain.info("expert.financial LLM: provider=\(provider.id, privacy: .public) available=false; using heuristic fallback")
            return ([], 0)
        }
        AtlasLog.brain.info("expert.financial LLM: provider=\(provider.id, privacy: .public) available=true")
        ...
        if let fmProvider = provider as? FoundationModelsProvider {
            do {
                ...
                AtlasLog.brain.info("expert.financial LLM: produced \(typed.count) typed claims via @Generable")
                return (typed, 0)
            } catch {
                AtlasLog.brain.error("expert.financial LLM: typed path failed (\(String(describing: error), privacy: .public)); falling back to prompt-parse")
            }
        }
        do {
            ...
            AtlasLog.brain.info("expert.financial LLM: provider=\(provider.id, privacy: .public) produced \(parsed.claims.count) claims, dropped \(parsed.dropped)")
            ...
            AtlasLog.brain.error("expert.financial LLM: provider=\(provider.id, privacy: .public) call failed → \(String(describing: error), privacy: .public)")
            return ([], 0)
        }
    }
}
