...
public struct FinancialExpert: Expert {
    ...
    private func runLLM(
        ...
        guard await provider.isAvailable() else {
            AtlasLog.brain.info("expert.financial LLM: provider resolved but isAvailable()=false; using heuristic fallback")
            return ([], 0)
        }
        do {
            ...
            AtlasLog.brain.info("expert.financial LLM: produced \(parsed.claims.count) claims, dropped \(parsed.dropped)")
