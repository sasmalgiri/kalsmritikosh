...
enum PersonaWorkCatalog {
    ...
    private static let legal: [PersonaWork] = [
        ...
        .init(id: "legal.matrix", title: "Compare every document at once",
              subtitle: "One question across all files — what each says, verbatim and cited.",
              icon: "square.grid.3x3.topleft.filled", action: .open(.matrix),
              extraKeywords: "cross document matrix clause compare across"),
    ...
    private static let investigation: [PersonaWork] = [
        ...
        .init(id: "inv.timeline", title: "Full timeline of events",
              ...
              icon: "calendar.day.timeline.left", action: .open(.timeline)),
        .init(id: "inv.matrix", title: "What every source says about a topic",
              subtitle: "One query across all evidence — each document's own words, cited.",
              icon: "square.grid.3x3.topleft.filled", action: .open(.matrix),
              extraKeywords: "cross document matrix compare across"),
        .init(id: "inv.connections", title: "Connect the dots between two people",
              subtitle: "The shortest chain of relationships linking two entities, each hop cited.",
              icon: "point.topleft.down.to.point.bottomright.curvepath", action: .open(.connections),
              extraKeywords: "connection path link between network how related"),
    ...
    private static let journalism: [PersonaWork] = [
        ...
        .init(id: "jour.matrix", title: "What each document says",
              subtitle: "Ask across the whole dump — one row per source, verbatim + cited.",
              icon: "square.grid.3x3.topleft.filled", action: .open(.matrix),
              extraKeywords: "cross document matrix compare across sources"),
