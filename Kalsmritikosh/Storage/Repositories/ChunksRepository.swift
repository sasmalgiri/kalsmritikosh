...
public actor ChunksRepository {
    ...
    public func insertBatch(_ chunks: [Chunk]) async throws {
        for chunk in chunks {
            ...
            INSERT INTO chunks (id, object_id, ordinal, text, char_start, char_end, page_number, created_at, context_prefix, context_prefix_source, admit_embedding, evidence_block_id, block_kind, source_version_id, salience)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?);
            """, [
                .uuid(chunk.id),
                .uuid(chunk.objectID),
    ...
    public func findChunksMissingContextPrefix(limit: Int = 100) async throws -> [Chunk] {
        ...
        SELECT id, object_id, ordinal, text, char_start, char_end, page_number, created_at, context_prefix, context_prefix_source, evidence_block_id, block_kind, salience
        FROM chunks
