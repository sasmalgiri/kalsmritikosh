...
| `manifest.json` | `{ "formatVersion": 1, "files": { "<name>": "<sha256 hex>" } }` over every other file, INCLUDING `README.txt` (ninth audit — README is in the MANDATORY set). Verifiers REFUSE an unknown `formatVersion`, REFUSE a manifest that does not cover the mandatory file set (eighth audit), and REFUSE a directory whose contents differ from `manifest.files` + `manifest.json` — deleted-and-delisted files and unlisted additions both fail (tenth audit), and an ENUMERATION FAILURE fails too (eleventh audit — fail-closed, never skipped). Only the two specific macOS browsing artifacts are ignored: `.DS_Store` and AppleDouble `._*`; any OTHER hidden file is a smuggled file and fails. |
